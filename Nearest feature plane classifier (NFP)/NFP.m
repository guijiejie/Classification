function rate = NFP(Train_data,Train_label,Test_data,Test_label);
    ...
    for k = 1:nclasses
        ...
        for j = 1:length(index)
            for m =(j+1):length(index)
                for z =(m+1):length(index)
                    temp = PointToPlaneDistance(test,Train_data(index(j),:),Train_data(index(m),:),Train_data(index(z),:));
                    if temp < dist(k,1)
                        dist(k,1) = temp;
                    end
                end
